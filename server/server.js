...
mongoose.connection.once('open', async () => {
  ...
  // ── Официальный бот Whispr ──
  const botExists = await User.findOne({ username: 'whispr' });
  if (!botExists) {
    await User.create({
      username: 'whispr',
      displayName: 'Whispr',
      password: Math.random().toString(36), // недоступный пароль
      isBot: true,
      verified: true,
      isAdmin: false
    });
    console.log('🤖 Официальный бот @whispr создан');
  } else {
    await User.updateOne({ username: 'whispr' }, { isBot: true, verified: true });
  }

  // Добавляем бота всем существующим пользователям (кто ещё не имеет)
  await User.updateMany(
    { username: { $ne: 'whispr' }, contacts: { $ne: 'whispr' } },
    { $push: { contacts: 'whispr' } }
  );
  console.log('🤖 Бот добавлен всем пользователям');
});
...
io.on('connection', (socket) => {
  ...
  socket.on('register', async (data, cb) => {
    try {
      ...
      const user = await User.create({ username: username.toLowerCase(), displayName, password, contacts: ['whispr'] });
  ...
  socket.on('admin_broadcast', async (data, cb) => {
    const me = requireAuth(cb); if (!me) return;
    try {
      const admin = await User.findOne({ username: me });
      if (!admin?.isAdmin) return cb({ success: false, error: 'Нет прав' });
      const { text } = data;
      if (!text?.trim()) return cb({ success: false, error: 'Пустое сообщение' });

      // Получаем всех пользователей кроме бота и админа
      const allUsers = await User.find({ username: { $ne: 'whispr' }, isBot: { $ne: true } });

      let sent = 0;
      for (const user of allUsers) {
        const chatId = getChatId('whispr', user.username);
        const msg = await Message.create({
          chatId, chatType: 'direct',
          from: 'whispr', to: user.username,
          text: text.trim(), type: 'text',
          delivered: userSockets.has(user.username), read: false
        });
        // Если пользователь онлайн — доставляем сразу
        const recipientSocket = userSockets.get(user.username);
        if (recipientSocket) io.to(recipientSocket).emit('new_message', msg);
        // Убеждаемся что бот в контактах
        if (!user.contacts.includes('whispr')) {
          await User.updateOne({ username: user.username }, { $push: { contacts: 'whispr' } });
        }
        sent++;
      }

      await logAdminAction(me, 'admin_broadcast', 'all_users', `Рассылка: "${text.trim().slice(0, 80)}"`);
      cb({ success: true, sent });
    } catch (e) { console.error('admin_broadcast error:', e); cb({ success: false, error: 'Ошибка: ' + e.message }); }
  });
