...
export default function WhisprPro() {
  ...
  useEffect(() => {
    ...
    s.on('poll_updated', poll => { // NEW
      ...
      setMessages(p => ({...p, [key]: (p[key]||[]).map(m => m.pollId===String(poll._id||'') ? {...m, poll} : m)}));
    });
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-4 space-y-0.5 scrollbar-none">
          ...
          {(activeFolder==='all'||activeFolder==='direct')&&contacts.map((c,i)=>{
            ...
            <div key={c.username} onClick={()=>openDirectChat(c)}
              ...
              <div className="flex-1 min-w-0">
                <div className={`text-sm font-medium truncate flex items-center gap-1 ${activeChat?.type==='direct'&&activeChat.id===c.username?'text-white':'text-white/60'}`}>{c.displayName}{c.isBot&&<span className="text-[9px] font-semibold px-1 py-0.5 rounded" style={{background:`${T.b}25`,color:T.b}}>бот</span>}{!c.isBot&&c.verified&&<VerifiedBadge/>}</div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-white/25">
                  {activeChat.type==='direct'
                    ? activeChatData?.isBot
                      ? <span style={{color:T.b,fontSize:'12px'}} className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{background:T.b}}></span>Официальный аккаунт</span>
        ...
        <Modal onClose={()=>setShowAdminPanel(false)} wide>
          <div className="p-6">
            ...
            <div className="flex gap-2 mb-4">
              {[['users','Пользователи'],['broadcast','Рассылка'],['features','Функции'],['logs','История']].map(([t,label])=>(
                <button key={t} onClick={()=>setAdminTab(t)} className={`px-4 py-2 rounded-xl text-sm font-medium flex items-center gap-2 transition-all ${adminTab===t?'text-white/80':'text-white/25 hover:text-white/50'}`} style={adminTab===t?{background:`linear-gradient(135deg,${T.a}30,${T.b}20)`,border:'1px solid rgba(255,255,255,0.07)'}:{}}>
                  {t==='users'?<><Users className="w-4 h-4"/>{label}</>:t==='broadcast'?<><Radio className="w-4 h-4"/>{label}</>:t==='features'?<><Settings className="w-4 h-4"/>{label}</>:<><History className="w-4 h-4"/>{label}</>}
                </button>
              ))}
            </div>
            {adminTab==='users'&&(
              ...
              <div className="space-y-1.5">{allUsers.map(u=>(
                <div key={u.username} className="px-4 py-3 rounded-xl flex items-center justify-between" style={{background:u.isBlocked?'rgba(239,68,68,0.04)':'rgba(255,255,255,0.02)',border:u.isBlocked?'1px solid rgba(239,68,68,0.1)':'1px solid rgba(255,255,255,0.04)'}}>
                  ...
                  <div className="flex gap-1.5 flex-shrink-0 flex-wrap justify-end">
                    ...
                    <button onClick={()=>socketRef.current?.emit('admin_verify_user',{target:u.username,verify:!u.verified},res=>{if(res.success){setAllUsers(p=>p.map(x=>x.username===u.username?{...x,verified:!u.verified}:x));}else alert(res.error||'Ошибка');})} className={`px-2.5 py-1.5 rounded-lg text-xs flex items-center gap-1 transition-colors ${u.verified?'text-blue-300':'text-white/30'}`} style={{background:u.verified?'rgba(59,130,246,0.1)':'rgba(255,255,255,0.04)',border:u.verified?'1px solid rgba(59,130,246,0.2)':'1px solid rgba(255,255,255,0.06)'}} title={u.verified?'Убрать верификацию':'Верифицировать'}>✓ {u.verified?'Верифиц.':'Верифик.'}</button>
            ...
            {adminTab==='broadcast'&&(
              <div className="space-y-4">
                <div className="px-4 py-3 rounded-xl flex items-start gap-3" style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)'}}>
                  <Radio className="w-4 h-4 mt-0.5 flex-shrink-0" style={{color:T.b}}/>
                  <div>
                    <div className="text-white/70 text-sm font-medium">Официальный аккаунт @whispr</div>
                    <div className="text-white/30 text-xs mt-0.5">Сообщение придёт от бота всем пользователям в личный чат</div>
                  </div>
                </div>
                <textarea
                  value={broadcastText}
                  onChange={e=>setBroadcastText(e.target.value)}
                  placeholder="Введите текст новости или объявления..."
                  rows={5}
                  className="w-full px-4 py-3 rounded-xl text-white/70 text-sm outline-none placeholder-white/20 resize-none"
                  style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)'}}
                />
                <div className="flex items-center justify-between">
                  <span className="text-white/20 text-xs">{broadcastText.length} символов</span>
                  <button
                    disabled={!broadcastText.trim()||broadcastSending}
                    onClick={()=>{
                      if(!broadcastText.trim())return;
                      if(!confirm(`Отправить сообщение всем пользователям?\n\n"${broadcastText.slice(0,100)}${broadcastText.length>100?'...':''}"`))return;
                      setBroadcastSending(true);
                      socketRef.current?.emit('admin_broadcast',{text:broadcastText},res=>{
                        setBroadcastSending(false);
                        if(res.success){
                          showToast(`✅ Отправлено ${res.sent} пользователям`,'success');
                          setBroadcastText('');
                        } else {
                    ...
                    className="px-5 py-2.5 rounded-xl text-sm font-medium text-white flex items-center gap-2 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                    style={{background:broadcastText.trim()&&!broadcastSending?`linear-gradient(135deg,${T.a},${T.b})`:'rgba(255,255,255,0.08)'}}
                  >
                    <Radio className="w-4 h-4"/>
                    {broadcastSending?'Отправляю...':'Отправить всем'}
                  </button>
                </div>
              </div>
            )}
