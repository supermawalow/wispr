...
export default function WhisprPro() {
  ...
  // ── Сокет создаётся ОДИН РАЗ — без зависимости от currentUser ──
  useEffect(() => {
  ...
  }, []); // ← пустой массив! сокет создаётся один раз
    ...
    if (confirm(`Удалить ${targetUsername} из контактов?`)) {
      socket.emit('remove_contact', targetUsername, (response) => {
        if (response.success) {
          ...
          if (activeChat === targetUsername) setActiveChat(null);
        }
      });
    }
  };
  ...
  const handleOpenChat = (contactUsername) => {
    ...
    socket.emit('load_chat', contactUsername, (response) => {
      if (response.success) {
        setMessages(prev => ({ ...prev, [contactUsername]: response.messages }));
      }
    });
  };
  ...
  const handleSendMessage = (e) => {
    ...
    socket.emit('send_message', 
      ...
      (response) => { if (response.success) setInputMessage(''); }
    );
  };

  const handleTyping = () => {
    if (activeChat) socket.emit('typing', activeChat);
  };

  const loadAdminStats = () => {
    socket.emit('admin_get_stats', (response) => {
      if (response.success) setAdminStats(response.stats);
    });
    socket.emit('admin_get_users', (response) => {
      if (response.success) setAllUsers(response.users);
    });
  };
  ...
  const handleDeleteUser = (targetUsername) => {
    if (confirm(`УДАЛИТЬ пользователя ${targetUsername}? Это необратимо!`)) {
      socket.emit('admin_delete_user', targetUsername, (response) => {
        if (response.success) loadAdminStats();
      ...
      <div className="min-h-screen relative overflow-hidden flex items-center justify-center p-4">
        ...
        <div className="relative backdrop-blur-2xl bg-white/10 border border-white/20 rounded-3xl shadow-2xl p-8 w-full max-w-md">
          ...
          <form onSubmit={isRegistering ? handleRegister : handleLogin} className="relative space-y-4">
            ...
            {authError && (
              <div className="text-red-300 text-sm bg-red-500/20 px-3 py-2 rounded-lg">{authError}</div>
            )}
        ...
        <div className="p-4 border-b border-white/20">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-1">
              {currentUser?.isAdmin && (
                <button onClick={() => { setShowAdminPanel(true); loadAdminStats(); }}
                  className="p-2 rounded-lg bg-white/20 hover:bg-white/30 transition-colors" title="Админ панель">
                  <Shield className="w-4 h-4 text-white" />
                </button>
              )}
              <button onClick={handleLogout}
                className="p-2 rounded-lg bg-white/20 hover:bg-white/30 transition-colors" title="Выйти">
          ...
          <>
            <div className="p-4 backdrop-blur-2xl bg-white/10 border-b border-white/20 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <button onClick={() => setShowSidebar(!showSidebar)} className="lg:hidden p-2 rounded-lg bg-white/20 hover:bg-white/30">
                  ...
                  <div className="text-xs text-white/60">
                    {activeChatData?.isOnline ? (
                      <span className="flex items-center gap-1">
                        <Circle className="w-2 h-2 fill-green-400 text-green-400" />онлайн
                      </span>
                    ) : 'не в сети'}
                  </div>
                </div>
              ...
              {chatMessages.map((msg) => {
                ...
                return (
                  <div key={msg._id || msg.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-md rounded-2xl px-4 py-3 backdrop-blur-xl transition-all ${
                      isOwn ? 'bg-white/25 border border-white/30 text-white shadow-lg'
                            : 'bg-white/15 border border-white/25 text-white shadow-md'
                    }`}>
                      <div className="text-sm leading-relaxed">{msg.text}</div>
                      <div className="text-xs mt-1 text-white/60">{formatTime(msg.timestamp)}</div>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="bg-white/10 backdrop-blur-2xl border border-white/20 rounded-3xl p-6 w-full max-w-md m-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              ...
              {searchQuery.length >= 2 && searchResults.length === 0 && (
                <div className="text-center py-8 text-white/60">Ничего не найдено</div>
              )}
              {searchQuery.length < 2 && (
                <div className="text-center py-8 text-white/60">Введите минимум 2 символа</div>
              )}
            </div>
          </div>
        </div>
                  ...
                  <div key={user.username} className="px-4 py-3 bg-white/10 backdrop-blur-xl border border-white/20 rounded-xl flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-white/60">@{user.username} • {user.contactsCount} контактов</div>
                      </div>
                    </div>
